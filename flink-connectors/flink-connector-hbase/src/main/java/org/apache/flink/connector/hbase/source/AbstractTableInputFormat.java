...
abstract class AbstractTableInputFormat<T> extends RichInputFormat<T, TableInputSplit> {
	...
	public AbstractTableInputFormat(org.apache.hadoop.conf.Configuration hConf) {
		serializedConfig = HBaseConfigurationUtils.serializeConfiguration(hConf);
	}
	...
	protected org.apache.hadoop.conf.Configuration getHadoopConfiguration() {
		return HBaseConfigurationUtils.deserializeConfiguration(serializedConfig, HBaseConfigurationUtils.getHBaseConfiguration());
	}
	...
	public TableInputSplit[] createInputSplits(final int minNumSplits) throws IOException {
		...
		final Pair<byte[][], byte[][]> keys = conn.getRegionLocator(table.getName()).getStartEndKeys();
		...
		for (int i = 0; i < keys.getFirst().length; i++) {
			...
			final String regionLocation = conn.getRegionLocator(table.getName()).getRegionLocation(startKey, false).getHostnamePort();
